function [pca_SVM_cub_model, pca_SVM_cub_accuracy] = train_pca_SVM_cubic(trainingData)
%% cubic SVM, dim = 200
...
% Auto-generated by MATLAB on 14-Nov-2017 09:42:54
...
pca_SVM_cub_model.predictFcn = @(x) svmPredictFcn(predictorExtractionFcn(x));

% Add additional fields to the result struct
pca_SVM_cub_model.ClassificationSVM = classificationSVM;
pca_SVM_cub_model.About = 'This struct is a trained model exported from Classification Learner R2017b.';
pca_SVM_cub_model.HowToPredict = sprintf('To make predictions on a new predictor column matrix, X, use: \n  yfit = c.predictFcn(X) \nreplacing ''c'' with the name of the variable that is this struct, e.g. ''trainedModel''. \n \nX must contain exactly 200 columns because this model was trained using 200 predictors. \nX must contain only predictor columns in exactly the same order and format as your training \ndata. Do not include the response column or any columns you did not import into the app. \n \nFor more information, see <a href="matlab:helpview(fullfile(docroot, ''stats'', ''stats.map''), ''appclassification_exportmodeltoworkspace'')">How to predict using an exported model</a>.');
...
partitionedModel = crossval(pca_SVM_cub_model.ClassificationSVM, 'KFold', 5);

% Compute validation predictions
[validationPredictions, validationScores] = kfoldPredict(partitionedModel);

% Compute validation accuracy
pca_SVM_cub_accuracy = 1 - kfoldLoss(partitionedModel, 'LossFun', 'ClassifError');
